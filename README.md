...
除了 Python 命令行版本，MarkIt 还提供 Chrome 浏览器扩展版本，替换新标签页，直接读取浏览器书签。
...
- 点击同步按钮（🔄）手动刷新浏览器书签
- 设置中可开启每小时自动同步
- 数据存储在 `chrome.storage.local`，跨标签页共享
- 所有原版功能完整保留：多视图、搜索、收藏、稍后阅读、去重等
...
|------|------|
| `bookmarks` | 读取浏览器书签数据 |
| `storage` | 存储用户偏好和自定义数据 |

扩展不需要联网权限，所有数据保存在本地。favicon 图标通过 Google 公共服务加载（`www.google.com/s2/favicons`），这是浏览器正常的网络请求，不需要额外权限。
